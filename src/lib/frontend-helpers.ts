...
import { schedules, timetables, classes, teachers, subjects, classrooms } from '../db/schema'
import { cachedQuery } from './cache-system'
import { getOptimizedSchedules, getBatchClassStatistics } from './query-optimizer'
...
export interface TimetableStatistics {
  ...
  emptySlots: number
  fillRate: number
  classCoverage: Record<string, {
    totalSlots: number
    filledSlots: number
    fillRate: number
  }>
  teacherUtilization: Record<string, {
    totalAssignedSlots: number
    utilizationRate: number
  }>
  classroomUtilization: Record<string, {
    totalAssignedSlots: number
    utilizationRate: number
  }>
    ...
    const classScheduleData = Array.from(classesMap.values()).map(classData => {
      const scheduleGrid: ScheduleGridCell[][] = Array.from({ length: 6 }, (_, dayIndex) =>
        Array.from({ length: 6 }, (_, periodIndex) => {
          const schedule = classData.schedules.find((s: any) => 
            s.schedule.dayOfWeek === dayIndex + 1 && 
            s.schedule.period === periodIndex + 1
          )
...
export async function getTimetableStatistics(
  ...
  // クラス別カバレッジ
  const classCoverage: Record<string, { totalSlots: number; filledSlots: number; fillRate: number }> = {}
  ...
  // 教師利用率
  const teacherUtilization: Record<string, { totalAssignedSlots: number; utilizationRate: number }> = {}
  const teacherScheduleCounts = allSchedules.reduce((acc, schedule) => {
    if (schedule.teacher) {
  ...
  // 教室利用率
  const classroomUtilization: Record<string, { totalAssignedSlots: number; utilizationRate: number }> = {}
  const classroomScheduleCounts = allSchedules.reduce((acc, schedule) => {
  ...
  // 教科分布
  const subjectDistribution = allSchedules.reduce((acc, schedule) => {
    if (schedule.subject) {
