...
export const ErrorSchema = z.object({
  ...
  details: z.array(z.string()).optional().openapi({ example: ['名前は必須です'] })
})
...
export const ConstraintDefinitionSchema = z.object({
  ...
  description: z.string().openapi({ example: '同一教師が同一時間に複数のクラスを担当することを禁止します' }),
  category: z.enum(['teacher', 'classroom', 'time', 'subject', 'custom']).openapi({ example: 'teacher' }),
  enabled: z.boolean().openapi({ example: true }),
  priority: z.number().int().min(1).max(10).openapi({ example: 10 }),
  parameters: z.record(z.any()).optional(),
  applicableSchoolTypes: z.array(z.string()).optional().openapi({ example: ['小学校', '中学校', '高校'] }),
  version: z.string().openapi({ example: '1.0.0' })
})

export const ConstraintViolationSchema = z.object({
  code: z.string().openapi({ example: 'teacher_conflict_TIME_CONFLICT' }),
  message: z.string().openapi({ example: '教師「山田太郎」が 1曜日 1時限に複数クラス（1年A組、1年B組）を担当しています' }),
  severity: z.enum(['error', 'warning', 'info']).openapi({ example: 'error' }),
  affectedSchedules: z.array(z.object({
    classId: z.string(),
    dayOfWeek: z.number(),
    period: z.number(),
...
export const BulkGenerateRequestSchema = z.object({
  classIds: z.array(z.string()).min(1, '対象クラスIDは必須です').openapi({ 
  ...
  globalConstraints: z.object({
    ...
    subjectDistribution: z.enum(['balanced', 'concentrated', 'flexible']).default('balanced').openapi({ example: 'balanced' })
  }).optional(),
  requirements: z.string().optional().openapi({ 
    example: '体育は午前中に配置してください',
    description: '追加の特別要求事項'
  }),
  priority: z.enum(['speed', 'quality', 'balanced']).default('balanced').openapi({ example: 'balanced' })
})
