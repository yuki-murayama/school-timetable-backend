...
import { userRoles, userSessions, users } from '../db/schema'
import type { Auth0User, AuthContext, UserRole } from './auth0'
...
export async function syncUserWithDatabase(
  ...
  return await cachedQuery(
    ...
    async () => {
      // 既存ユーザーを検索
      let dbUser = await db.select().from(users).where(eq(users.auth0Id, auth0User.sub)).get()
...
export async function createUserSession(
  ...
  const session = await db.insert(userSessions).values(sessionData).returning().get()
...
export async function getUserActiveSessions(
  ...
  const sessions = await db
    .select()
    .from(userSessions)
    .where(and(eq(userSessions.auth0Id, auth0Id), eq(userSessions.isActive, true)))
...
export async function getSchoolUsers(
  ...
  const queryFn = async () => {
    let query = db
      .select()
      .from(users)
      .where(and(eq(users.schoolId, schoolId), eq(users.isActive, isActive)))
...
export async function getUserStatistics(
  ...
  return await cachedQuery(
    ...
    async () => {
      ...
      const recentLogins = activeUsers.filter(u => u.lastLoginAt && u.lastLoginAt > yesterday)
...
export async function validateUserIntegrity(
  ...
  try {
    const dbUser = await db.select().from(users).where(eq(users.auth0Id, auth0Id)).get()
