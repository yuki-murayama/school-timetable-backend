...
export class TimeSlotPreferenceValidator extends BaseConstraintValidator {
  ...
  async validate(context: ConstraintContext): Promise<ConstraintValidationResult> {
    const { result, executionTime } = await this.measurePerformance(async () => {
      const violations = []
      const { morningSubjects, afternoonSubjects, strictMode, morningPeriods, afternoonPeriods } = this.definition.parameters!
      ...
      for (const subjectId of morningSubjects) {
        ...
        if (afternoonSchedules.length > 0) {
          ...
          const classNames = classIds.map(id => 
            context.metadata.classes.find(c => c.id === id)?.name || id
          )
      ...
      // 時間帯バランス分析（情報提供）
      const timeSlotAnalysis = this.analyzeTimeSlotDistribution(context.schedules, morningPeriods, afternoonPeriods)
      if (timeSlotAnalysis.imbalance > 0.3) { // 30%以上の偏りがある場合
        violations.push(this.createViolation(
          'TIME_SLOT_IMBALANCE',
  ...
  validateParameters(parameters: Record<string, any>): { isValid: boolean; errors: string[] } {
    ...
    if (!Array.isArray(parameters.morningPeriods) || !parameters.morningPeriods.every((p: any) => typeof p === 'number')) {
      errors.push('morningPeriods は数値の配列である必要があります')
    }

    if (!Array.isArray(parameters.afternoonPeriods) || !parameters.afternoonPeriods.every((p: any) => typeof p === 'number')) {
