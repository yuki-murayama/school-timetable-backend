...
app.patch('/:constraintId', 
  zValidator('json', UpdateConstraintSchema),
  (c) => {
    try {
      const constraintId = c.req.param('constraintId')
...
app.post('/validate/:timetableId',
  zValidator('json', ValidateConstraintsSchema),
  async (c) => {
    try {
      const timetableId = c.req.param('timetableId')
      const { schedules, schoolId, saturdayHours, enabledConstraints, skipConstraints } = c.req.valid('json')
      
      const db = getDb(c.env.DB)
      
      const validationResult = await validateTimetableConstraints(
        db,
        timetableId,
        schedules,
        {
...
// 制約カテゴリ別の検証
app.post('/validate/:timetableId/category/:category',
  zValidator('json', ValidateConstraintsSchema),
  async (c) => {
    try {
      const timetableId = c.req.param('timetableId')
      const category = c.req.param('category') as 'teacher' | 'classroom' | 'time' | 'subject' | 'custom'
