...
export const bulkGenerateTimetableRoute = createAPIRoute({
  ...
  tags: ['Timetables'],
  summary: 'バルク時間割生成',
  description: '複数クラスの時間割を一括生成します。AIを使用して制約条件を満たす時間割を自動生成します。',
  request: {
    params: z.object({
      id: z.string().openapi({ 
        ...
        'application/json': {
          schema: z.object({
            ...
            data: z.object({
              ...
              schedules: z.array(ScheduleSchema.extend({
                subject: z.object({ id: z.string(), name: z.string() }).optional(),
                teacher: z.object({ id: z.string(), name: z.string() }).optional(),
                classroom: z.object({ id: z.string(), name: z.string(), type: z.string() }).optional()
              }))
            })
          })
        }
