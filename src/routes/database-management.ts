...
app.post('/timetables/:timetableId/snapshot', zValidator('json', CreateSnapshotSchema), async c => {
...
app.put(
  '/schools/:schoolId/constraints/:constraintType',
...
app.put('/system/settings/:key', zValidator('json', SetSystemSettingSchema), async c => {
  const key = c.req.param('key')
...
app.post('/schools/:schoolId/access', zValidator('json', GrantAccessSchema), async c => {
  const schoolId = c.req.param('schoolId')
...
app.post('/generation-logs', zValidator('json', LogGenerationSchema), async c => {
