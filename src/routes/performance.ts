...
import { z } from 'zod'
import { cleanupAllCaches, clearAllCaches, getAllCacheStats, getCache } from '../lib/cache-system'
...
app.get('/statistics', zValidator('query', StatisticsQuerySchema), c => {
...
app.get('/metrics', zValidator('query', MetricsQuerySchema), c => {
...
app.get('/cache/statistics', c => {
  ...
  try {
    ...
    const totalMemoryUsage = Object.values(cacheStats).reduce(
...
app.post('/cache/:cacheName/clear', zValidator('json', CacheActionSchema), c => {
...
app.post('/benchmark', zValidator('json', BenchmarkSchema), async c => {
