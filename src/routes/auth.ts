...
app.post('/mock/user', zValidator('json', MockUserSchema), c => {
  const requestId = c.get('requestId')
...
app.get('/config', c => {
  ...
  return c.json({
    ...
    data: {
      ...
      loginUrl: process.env.AUTH0_DOMAIN ? `https://${process.env.AUTH0_DOMAIN}/authorize` : null,
      logoutUrl: process.env.AUTH0_DOMAIN ? `https://${process.env.AUTH0_DOMAIN}/v2/logout` : null,
    },
    requestId,
  })
})
